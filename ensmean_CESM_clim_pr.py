...
def process_run(experiment,runpath1,runpath2,run_whole):
	...
	# CDO command
	cdo_cmd = 'cdo ymonmean -add '+seldate + runpath1 + ' ' + seldate + runpath2 + ' ' + run_whole
	print cdo_cmd
	os.system(cdo_cmd)
...
def process_data(experiment,var,basepath,numthreads):
	...
	try:		
		...
		for runpath in runs:
			...
			run_whole = os.path.join(temp_dir,experiment+'_'+var+'_'+run)
			# Add the process to the pool
			runpath2 = runpath.replace('PRECL','PRECC')
			pool.apply_async(process_run,(experiment,runpath,runpath2,run_whole))
			#process_run(experiment,runpath,runpath2,run_whole)

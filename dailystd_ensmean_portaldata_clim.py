...
def process_run(runpath,run_whole):
	...
	if len(run_files)==0:
		...
		# Only one file
		# CDO command
		cdo_cmd = 'cdo ymonstd ' + run_files[0] + ' ' + run_whole
		print cdo_cmd
		os.system(cdo_cmd)
	else: 
		# concatenate files
		# CDO command
		cdo_cmd = "cdo ymonstd -cat '" + list_to_string(run_files) + "' "+ run_whole
		print cdo_cmd
		os.system(cdo_cmd)
...
def process_data(model,experiment,var,basepath,numthreads):
	...
	try:		
		...
		outmean = '/export/silurian/array-01/pu17449/processed_data_clim/'+model+'.'+var+'.'+experiment+'_daystd_ensmean.nc'
		outstd = '/export/silurian/array-01/pu17449/processed_data_clim/'+model+'.'+var+'.'+experiment+'_daystd_ensstd.nc'

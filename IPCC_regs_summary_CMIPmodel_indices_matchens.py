...
from bootstrapping import bootstrap_range
...
if __name__=='__main__':
	...
	# indices of experiments (second minus first)
	scenario_indices = [(0,1),(0,2),(1,2)]
	...
	if host[:6] == 'jasmin' or host[-11:] == 'jc.rl.ac.uk':
		...
		cmip_summary_pkl = '/home/users/pfu599/pkl/'+dataset+'models_'+index+'_IPCCregs_jasmin_summary_matchens.pkl'
		summary_pkl = '/home/users/pfu599/pkl/'+index+'_IPCCregs_jasmin_summary.pkl'
		summary_name = dataset+'-weighted2'
	else:
	...
	for z,model in enumerate(models):
		...
		for reg in regs:
			expt_data = []
			# Flatten data for this model/region into 'expt_data' array
			for experiment in experiments:
				expt_data.append(data_masked[model][experiment][reg])
		
			for d,scen in enumerate(scenarios):
				scen_data = []
				# calculate bootstrapped error for mean:
				expindex1 = scenario_indices[d][1]
				for ens,data1 in expt_data[expindex1].items():
					expindex0 = scenario_indices[d][0]
					try:
						data0 = expt_data[expindex0][ens]
					except:
						continue # there isn't a matching ensemble member in both experiments
					scen_data.append((data1-data0)/data0*100.)
				scen_data = np.array(scen_data).flatten()
				print('Scen data',len(scen_data))
				#for s in scen_data: print(len(s))
				# Calculate the median trend and 5-95 percentiile confidence interval
				pct_change = bootstrap_range(scen_data,50.)
